# src/knowledge_mcp/ingest/docx_ingestor.py
"""
Docling-based DOCX ingestor.

This module provides a DOCX ingestor using Docling's DocumentConverter
for parsing DOCX files while preserving clause hierarchy, extracting tables
...
class DOCXIngestor(BaseIngestor):
    ...
    Converts DOCX documents to ParsedDocument objects using Docling's
    ...
    Example:
        ...
        >>> doc = ingestor.ingest(Path("requirements.docx"))
        >>> print(f"Parsed {len(doc.elements)} elements")
    """

    def __init__(self) -> None:
        """Initialize the DOCX ingestor with Docling converter."""
        self.converter = DocumentConverter()
        logger.debug("DOCXIngestor initialized with DocumentConverter")

    def ingest(self, file_path: Path) -> ParsedDocument:
        """
        Ingest a DOCX document and convert to ParsedDocument.
        ...
        try:
            ...
            logger.error(f"Failed to ingest DOCX {file_path}: {e}")
            raise IngestionError(f"Failed to parse DOCX: {e}") from e
