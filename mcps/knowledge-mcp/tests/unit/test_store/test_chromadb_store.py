...
def cleanup_chromadb_module() -> Generator[None, None, None]:
    """Clean up chromadb_store module after each test to prevent pollution."""
    yield
    # After test, remove cached module so next test gets fresh import
    if "knowledge_mcp.store.chromadb_store" in sys.modules:
        del sys.modules["knowledge_mcp.store.chromadb_store"]


def create_mock_chromadb() -> tuple[MagicMock, MagicMock, MagicMock]:
    """Create mock chromadb module with client and collection."""
...
class TestChromaDBStoreInit:
    ...
    def test_creates_directory_if_not_exists(self, mock_config: KnowledgeConfig) -> None:
        ...
        mock_chromadb, mock_client, mock_collection = create_mock_chromadb()
...
class TestChromaDBStoreSearch:
    ...
    def test_search_returns_formatted_results(self, mock_config: KnowledgeConfig) -> None:
    ...
    def test_search_with_filter_dict(self, mock_config: KnowledgeConfig) -> None:
